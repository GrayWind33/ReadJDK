...
abstract class AbstractStringBuilder implements Appendable, CharSequence {
     ...
     * </ul>
     * 如果当前容量小于参数值，分配一个新的更大的内部数组，他的大小是minimumCapacity和旧容量*2+2中的较大值
     * If the {@code minimumCapacity} argument is nonpositive, this
     * method takes no action and simply returns.
     * 如果minimumCapacity是负数，什么也不做直接返回
     ...
     * that suffices.
     * 返回不小于minCapacity的大小，如果当前大小*2+2足够的话就取该值。
     * Will not return a capacity greater than {@code MAX_ARRAY_SIZE}
     * unless the given minimum capacity is greater than that.
     * 不会返回超过MAX_ARRAY_SIZE的大小，除非minCapacity超过该值
     *
    ...
    public void setLength(int newLength) {
        ...
        ensureCapacityInternal(newLength);//newLength小于当前数组大小的话直接返回，否则分配一个大小为newLength和当前容量*2+2的较大值的新数组，并复制字符
     ...
     * 返回index位置的字符，第一个字符index是0，下一个是1，依次类推
     * <p>
     ...
     * one code point each.
     * 返回从index到codePointOffset的代码点偏移index。每个不成对的代理(两个代码单元表示一个代码点时称为两个代理)在范围内被记为一个代码点。
     *
    ...
    public AbstractStringBuilder replace(int start, int end, String str) {
        ...
        System.arraycopy(value, end, value, start + len, count - end);//复制end开始的位置，留出str插入的空间
        str.getChars(value, start);//复制str到value中对应的位置
        count = newCount;
        return this;
    }
