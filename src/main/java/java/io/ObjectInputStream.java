...
{
    /** handle value representing null处理值表示null */
    ...
    private static class Caches {
        /** cache of subclass security audit results子类安全审计结果缓存 */
        static final ConcurrentMap<WeakClassKey,Boolean> subclassAudits =
            new ConcurrentHashMap<>();

        /** queue for WeakReferences to audited subclasses对审计子类弱引用的队列 */
        static final ReferenceQueue<Class<?>> subclassAuditsQueue =
            new ReferenceQueue<>();
    }

    static {
        /* Setup access so sun.misc can invoke package private functions.创建权限，这样sun.misc可以调用包内的私有函数 */
    ...
    /*
     * Separate class to defer initialization of logging until needed.单独的类来推迟需要的日志单元初始化
     */
    private static class Logging {
         ...
         * 日志记录ObjectInputFilter结果。如果日志被设置为INFO或WARNING要建立过滤器（假设它不会变化）。
         */
    ...
    /** filter stream for handling block data conversion处理数据块转换的过滤流 */
    private final BlockDataInputStream bin;
    /** validation callback list确认调用返回列表 */
    private final ValidationList vlist;
    /** recursion depth递归深度 */
    private long depth;
    /** Total number of references to any type of object, class, enum, proxy, etc.对任何种类的对象、类、枚举、代理的引用总数 */
    private long totalObjectRefs;
    /** whether stream is closed流是否关闭 */
    private boolean closed;

    /** wire handle -> obj/exception map线柄->obj/exception映射 */
    private final HandleTable handles;
    /** scratch field for passing handle values up/down call stack传递句柄值上下调用栈的草稿区 */
    private int passHandle = NULL_HANDLE;
    /** flag set when at end of field value block with no TC_ENDBLOCKDATA当在字段值末尾因没有TC_ENDBLOCKDATA阻塞时设置的标志 */
    private boolean defaultDataEnd = false;

    /** buffer for reading primitive field values读取原始字段值的缓冲区 */
    private byte[] primVals;

    /** if true, invoke readObjectOverride() instead of readObject()如果为true，调用readObjectOverride()来替代readObject() */
     ...
     * @throws  StreamCorruptedException if the stream header is incorrect如果流头部错误
     * @throws  IOException if an I/O error occurs while reading stream header在读取流头部是发生了IO错误
     * @throws  SecurityException if untrusted subclass illegally overrides
     *          security-sensitive methods如果不被信任的子类非法重写了安全敏感方法
     * @throws  NullPointerException if <code>in</code> is <code>null</code>如果in是null
    ...
    private void verifySubclass() {
        ...
        if (cl == ObjectInputStream.class) {
            return;//该类不是ObjectInputStream的子类直接返回
        }
        SecurityManager sm = System.getSecurityManager();
        if (sm == null) {
            return;//没有安全管理器直接返回
        }
        processQueue(Caches.subclassAuditsQueue, Caches.subclassAudits);//从子类安全审计结果缓存中移除对审计子类弱引用的队列内的类
        WeakClassKey key = new WeakClassKey(cl, Caches.subclassAuditsQueue);//创建一个对给定类对象的弱键，注册对审计子类弱引用队列
        Boolean result = Caches.subclassAudits.get(key);//检查子类安全审计结果缓存中是否有这个类生成的弱键
        if (result == null) {//如果缓存中没有该类的结果，则检查该类有没有重写安全敏感方法，并缓存该结果
            result = Boolean.valueOf(auditSubclass(cl));
            Caches.subclassAudits.putIfAbsent(key, result);
        }
        if (result.booleanValue()) {//子类安全则返回
            return;
        }
        sm.checkPermission(SUBCLASS_IMPLEMENTATION_PERMISSION);//检查当前安全策略下是否允许子类重写安全敏感方法，若不能则抛出异常
    }
            ...
            new PrivilegedAction<Boolean>() {
                public Boolean run() {
                    for (Class<?> cl = subcl;
                         ...
                         cl = cl.getSuperclass())//cl从底向上遍历至ObjectInputStream
                    {
                        try {
                            cl.getDeclaredMethod(
                                "readUnshared", (Class[]) null);//检查有没有重写readUnshared
                            return Boolean.FALSE;
                        } catch (NoSuchMethodException ex) {
                        }
                        try {
                            cl.getDeclaredMethod("readFields", (Class[]) null);//检查有没有重写readFields
                            return Boolean.FALSE;
                        } catch (NoSuchMethodException ex) {
                        }
                    }
    ...
    private static class PeekInputStream extends InputStream {
        ...
        /** underlying stream下层流 */
        private final InputStream in;
        /** peeked byte被取数的字节 */
        private int peekb = -1;
        /** total bytes read from the stream从流读取的总字节 */
        private long totalBytesRead = 0;

        /**
         * Creates new PeekInputStream on top of given underlying stream.
         * 在给与的下层输入流上端创建一个新的取数输入流
         */
         ...
         * 取数流中下一个字节的值。跟read()相似，除了取数不会消费这个读取值也就是说不会移动下标
         */
        int peek() throws IOException {
            if (peekb >= 0) {
                return peekb;//peekb有存储的字节则直接返回
            }
            peekb = in.read();//从下层输入流读取一个字节存储到peekb中
            totalBytesRead += peekb >= 0 ? 1 : 0;//读取到字节则增加totalBytesRead
            return peekb;
        }

        public int read() throws IOException {
            if (peekb >= 0) {
            	//peekb有存储的字节则将值设为-1并返回存储值
                int v = peekb;
                peekb = -1;
                return v;
            } else {
                //peekb没有存储的字节，从下层输入流读取一个，如果读取成功增加
            	int nbytes = in.read();
        ...
        /**
         * 从下层流读取长度len的字节到数组b从off开始的位置，如果有的话会先消耗peekb中的字节，如果流中内容不足会提前结束，返回实际读取的字节数
         */
        public int read(byte[] b, int off, int len) throws IOException {
            ...
            if (len == 0) {
                ...
                //peekb没有存储字节则直接从下层输入流读取所需的内容并返回
            	nbytes = in.read(b, off, len);
                totalBytesRead += nbytes >= 0 ? nbytes : 0;
                return nbytes;
            } else {
                //peekb有存储字节时先消耗掉这个字节然后再从下层输入流读取所需的字节
            	b[off++] = (byte) peekb;
                len--;
                peekb = -1;
                nbytes = in.read(b, off, len);
        ...
        /**
         * 从下层流读取长度len的字节到数组b从off开始的位置，不考虑peekb的存储内容，下层流中内容不够会抛出异常
         */
        ...
        /**
         * 跳过n字节，若peekb有存储内容会先跳过它，然后使用下层输入流的skip方法，返回实际跳过的字节数，不能反向跳过
         */
        public long skip(long n) throws IOException {
            ...
            n = skipped + in.skip(n);//调用下层流的skip方法
            totalBytesRead += n;//增加totalBytesRead
            return n;
        }

        /**
         * 检查下层输入流和peekb中有没有可读取的字节
         */
        public int available() throws IOException {
     ...
     * 输入流有两个模式：默认模式下，输入数据写入的格式和DataOutputStream相同；在块数据模式，输入的数据被块数据标记归为一类
     */
        ...
        extends InputStream implements DataInput
    {
         ...
         * 在给出的下层流上创建一个BlockDataInputStream。块数据模式默认是关闭的。
         */
         ...
         * 将块数据模式设为给定的模式，true是打开，off是关闭，并返回之前的模式值。
         * 如果新的模式和旧模式一样，什么都不做。
         * 如果块数据模式从打开被改为关闭时有未消费的块数据还留在流中，会抛出IllegalStateException
         */
         ...
         * 如果在块数据模式，跳跃到数据块的当前组的末尾但不会取消块数据模式。如果不在块数据模式，抛出IllegalStateException
         */
         ...
         * 尝试读取下一个块数据头部。如果canBlock是false并且一个完整的头部没有阻塞时不能被读取，返回HEADER_BLOCKED。
         * 否则如果流中下一个元素是一个块数据头部，返回头部标识的这个块数据的长度，否则返回-1
         */
         ...
         * 用块数据再装满内部缓冲区。任何buf中的数据在调用这个方法时被认为是已经被消费了。
         * 设置pos、end、unread字段值来反映有效块数据的数量。
         * 如果流中的下一个元素不是一个数据块，设置pos=0和unread=0，end=-1
         */
