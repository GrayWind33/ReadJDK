    ...
    implements NavigableMap<K,V>, Cloneable, java.io.Serializable
{
     ...
     * 获取key值对应的键值对，如果没有则返回key大于他的键值对里key最小的那个，还没有就返回null
     */
     ...
     * 返回key值对应的键值对，如果没有就返回小于的结点里key值最大的那个，再没有就返回null
     */
     ...
     * 返回key小于参数key的结点中最大的一个，没有则返回null
     */
     ...
     * value is replaced.
     * 改变已有结点的value不算一次结构性变化，modCount不变
     *
     ...
     * @throws ClassCastException if the specified key cannot be compared
     *         with the keys currently in the map key是不可比较类时抛错
     * @throws NullPointerException if the specified key is null
     *         and this map uses natural ordering, or its comparator
     *         does not permit null keys key是null时
     */
    public V put(K key, V value) {
        Entry<K,V> t = root;
        if (t == null) {//当前集合为空
            compare(key, key); //检查key是否可比较
        ...
        Comparator<? super K> cpr = comparator;
        if (cpr != null) {//comparator不为null时，用comparator来比较key
            do {
                parent = t;
                ...
                if (cmp < 0)
                    ...
                    return t.setValue(value);//寻找到了key值相等的点
            } while (t != null);
        }
        else {
            if (key == null)
            ...
            @SuppressWarnings("unchecked")
                Comparable<? super K> k = (Comparable<? super K>) key;//key是非可比较类时会报错
            do {
                parent = t;
        ...
        Entry<K,V> e = new Entry<>(key, value, parent);//没有key相等的结点，新建一个，作为parent的子结点
        if (cmp < 0)
            parent.left = e;
        else
            parent.right = e;
        fixAfterInsertion(e);//以插入的结点为中心，修复维持红黑树性质
        size++;
        modCount++;
        return null;
    }
    ...
    abstract static class NavigableSubMap<K,V> extends AbstractMap<K,V>
         ...
         * fromStart为true，返回map的下界是m的下界，否则若loInclusive为true，lo是包含在内的下界。若loInclusive为false，lo是不包含在内的下界
         * 上届的确定相同
         */
        final K lo, hi;
     ...
     *        the iterator or stream 数据个数
     * @param it If non-null, new entries are created from entries
     *        or keys read from this iterator.
     * @param str If non-null, new entries are created from keys and
     *        possibly values read from this stream in serialized form.
     *        Exactly one of it and str should be non-null. it和str至少一个应该是非null的
     * @param defaultVal if non-null, this default value is used for
     *        each value in the map.  If null, each value is read from
     *        iterator or stream, as described above. 非null时，map中的所有value都是该值，为null则从迭代器或流中读取value值
     * @throws java.io.IOException propagated from stream reads. This cannot
     *         occur if str is null. str为null时不会有这个错误
     * @throws ClassNotFoundException propagated from readObject.
     *         This cannot occur if str is null. str为null时不会有这个错误
     */
     ...
     *
     * @param level the current level of tree. Initial call should be 0.树的level初始为0
     * @param lo the first element index of this subtree. Initial should be 0.子树第一个点的下标，初始为0
     * @param hi the last element index of this subtree.  Initial should be
     *        size-1.子树最后一个点的下标，初始为size-1
     * @param redLevel the level at which nodes should be red.
     *        Must be equal to computeRedLevel for tree of this size.红结点的level应该与computeRedLevel(size)相等
     */
    @SuppressWarnings("unchecked")
    private final Entry<K,V> buildFromSorted(int level, int lo, int hi,
        ...
        // 将非满的最底层结点染红，递归过程中redLevel没变也就是说只有一个高度会出现红色
        if (level == redLevel)
            middle.color = RED;
