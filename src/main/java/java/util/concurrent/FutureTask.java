...
public class FutureTask<V> implements RunnableFuture<V> {
     ...
     *  get()操作返回的结果或者抛出的异常*/
    private Object outcome; // non-volatile, protected by state reads/writes不是volatile，由reads/writes状态来保护
     ...
     * 引起future报告一个ExecutionException并给出它的Throwable原因，除非这个future已经被set或者取消
     *
     * <p>This method is invoked internally by the {@link #run} method
     * upon failure of the computation.
     * 这个方法在run方法内部计算失败时被调用
     *
     ...
     * nulls out callable.
     * 移除并通知所有等待的线程，调用done()，并将callable设为null
     */
     ...
     * 等待完成或者因中断而终止或者超时
     *
     * @param timed true if use timed waits如果是限时等待为true
     * @param nanos time to wait, if timed如果是限时等待，等待的纳秒
     ...
     * schemes.
     * 尝试取消连接一个超时或者中断的等待结点避免积累垃圾。内部结点可以简单地不通过CAS取消拼接，
     * 因为如果它们被释放着以任何方式浏览都是无害的。为了避免取消拼接已经移除结点的影响，再次遍历表以防表面竞争。
     * 有很多结点时这个方法很慢，但是我们不期望列表足够长来超过更高开销的方案。
     */
    private void removeWaiter(WaitNode node) {
        if (node != null) {
            node.thread = null;
            retry:
            for (;;) {          // restart on removeWaiter race重新开始removeWaiter竞争
