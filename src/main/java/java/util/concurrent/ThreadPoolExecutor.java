...
public class ThreadPoolExecutor extends AbstractExecutorService {
     ...
     * 主要线程控制状态ctl是一个原子整数包装了两个概念性字段：workerCount有效线程数，runState是运行还是关闭状态。
     *
     ...
     * 为了将它们包装到一个int中，我们限制workerCount最大到(2^29)-1大约500万而不是(2^31)-1大约2亿。
     * 如果未来这成为了一个问题的话，可以调整它为AtomicLong并修改位移和掩码常量。
     * 但是在需求上升之前，代码使用int更加快速和简单。
     *
     ...
     * workerCount是工作者的数量，允许开始不允许停止。它的值可能有片刻和存活线程数不同，
     * 比如当一个ThreadFactory创建线程失败时，当存在线程终止前还在进行记账时。
     * 用户可见的池大小是工作者集的当前大小。
     *
     ...
     *             不接受新的任务，也不处理队列中的任务，中断处理中的任务
     *   TIDYING:  All tasks have terminated, workerCount is zero,
     *             the thread transitioning to state TIDYING
     *             will run the terminated() hook method
     *             所有任务都终止了，workerCount为0，线程变更到TIDYING状态会运行terminated()方法
    ...
    // Packing and unpacking ctl包装和拆装ctl
        ...
        implements Runnable
    {
        ...
        /**
         * Creates with given first task and th时read from ThreadFactory.
         * @param firstTask the first task (null if none)
         */
        Worker(Runnable firstTask) {
            setState(-1); // inhibit interrupts until runWorker直到runWorker前禁止打断
     ...
     * 检查是否有一个新的worker可以在遵守当前线程池状态和给出的边界(core或maximum)被添加。
     * 如果可以，调整worker计数器并且创建启动一个新的worker，运行firstTask作为它的第一个任务。
     * 如果池被停止或者符合关闭条件，这个方法返回false。如果线程工厂创建线程失败，也返回false。
     * 如果线程因为线程工厂返回null或者抛出异常(特别是OutOfMemoryError)，需要回滚。
     *
    ...
    private void addWorkerFailed(Worker w) {
        ...
        try {
            if (w != null)
                workers.remove(w);
            decrementWorkerCount();//减小worker计数器
            tryTerminate();
        } finally {
            mainLock.unlock();
        }
     ...
     * 阻塞或者限时等待一个任务，根据当前的设定，如果worker因为以下情况退出返回null:
    ...
    private Runnable getTask() {
        ...
        for (;;) {
            ...
            // Are workers subject to culling?workers是否需要裁剪
     ...
     * 主要worker运行循环。重复从队列获取任务并执行，同时复制一系列issue
     *
     ...
     * 我们可能由一个初始任务开始，在这种情况下我们不需要获得第一个任务。否则，只要线程池还在运行，我们通过getTask获取任务。
     * 如果这个方法返回null，worker会因为线程状态改变或者调整参数而退出。其他的退出情况还有内部代买抛出异常，
     * 异常由completedAbruptly持有，导致线程发生processWorkerExit。
     *
     * 2. Before running any task, the lock is acquired to prevent
     * other pool interrupts while the task is executing, and then we
     * ensure that unless pool is stopping, this thread does not have
     * its interrupt set.
     * 在运行任何任务之前，要请求锁来避免其他池在任务执行期间打断，所以可以确定除非池停止，线程不会被打断。
     *
     * 3. Each task run is preceded by a call to beforeExecute, which
     * might throw an exception, in which case we cause thread to die
     * (breaking loop with completedAbruptly true) without processing
     * the task.
     * 每个任务运行前要调用beforeExecute，可能会抛出异常，这种情况下线程不会执行任务而死去。
     *
     ...
     * 假设beforeExecute正常完成，我们运行任务手机所有抛出的异常发送给afterExecute。
     * 分别处理RuntimeException和Error以及任意Throwables。因为我们不能再Runnable.run中重复抛出Throwables，
     * 要在Errors内包住。任何抛出的异常会引起线程死亡。
     *
     * 5. After task.run completes, we call afterExecute, which may
     * also throw an exception, which will also cause thread to
     * die. According to JLS Sec 14.20, this exception is the one that
     * will be in effect even if task.run throws.
     * 在task.run完成之后，调用afterExecute，这个方法可能会抛出异常引起线程死亡。、
     * 根据JLS Sec 14.20，即使take.run抛出异常，这个异常也会生效。
     *
     ...
     * 使用给定的初始化参数和默认的线程工厂以及默认的拒绝策略创建一个新的ThreadPoolExecutor。
     * 可能使用Executors工厂方法来替代一般的构造函数更加方便。
     *
     ...
     * @param corePoolSize the number of threads to keep in the pool, even
     *        if they are idle, unless {@code allowCoreThreadTimeOut} is set
     *        池内保持的空闲线程，除非设置了allowCoreThreadTimeOut
     * @param maximumPoolSize the maximum number of threads to allow in the
     *        pool
     *        池内允许的最大线程数
     * @param keepAliveTime when the number of threads is greater than
     *        the core, this is the maximum time that excess idle threads
     *        will wait for new tasks before terminating.
     *        线程数量超过核心数时，在被终止前超过的空闲的线程将会等待新任何的最大时间
     * @param unit the time unit for the {@code keepAliveTime} argument
     *        keepAliveTime参数的时间单位
     * @param workQueue the queue to use for holding tasks before they are
     *        executed.  This queue will hold only the {@code Runnable}
     *        tasks submitted by the {@code execute} method.
     *        在任务被执行前持有它们的队列，这个队列只持有由execute方法提交的Runnable任务
     * @param threadFactory the factory to use when the executor
     *        creates a new thread
     *        执行创建一个新的线程时使用的工厂
     * @param handler the handler to use when execution is blocked
     *        because the thread bounds and queue capacities are reached
     *        执行因为线程范围和队列容量到达被阻塞时使用的处理器
     * @throws IllegalArgumentException if one of the following holds:<br>
     *         {@code corePoolSize < 0}<br>
     *         {@code keepAliveTime < 0}<br>
     *         {@code maximumPoolSize <= 0}<br>
