...
public class LinkedBlockingQueue<E> extends AbstractQueue<E>
    ...
    /** The capacity bound, or Integer.MAX_VALUE if none 容量上限，不设置的话是整数最大值*/
    ...
    /** Wait queue for waiting takes 等待获取元素的队列*/
    private final Condition notEmpty = takeLock.newCondition();

    /** Lock held by put, offer, etc 入队锁*/
    private final ReentrantLock putLock = new ReentrantLock();

    /** Wait queue for waiting puts 等待放入元素的队列*/
...
//     }
    ...
    public LinkedBlockingQueue(Collection<? extends E> c) {
        this(Integer.MAX_VALUE);
        final ReentrantLock putLock = this.putLock;
        putLock.lock(); // Never contended, but necessary for visibility没有竞争，但是为了可见性要加锁
        try {
            int n = 0;
            for (E e : c) {
                if (e == null)
	...
	public void put(E e) throws InterruptedException {
	    ...
	    try {
	         ...
	         * 尽管count没有被锁保护，但它却能用来作为等待的监视，因为count只在这里或者其他put操作中减少。
	         * 并且我们会被通知它改变了。其他使用count作为等待监视也是同样的。
	         */
